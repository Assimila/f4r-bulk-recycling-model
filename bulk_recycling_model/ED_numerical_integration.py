...
def np_trapz_with_extrapolation(integrand: np.ndarray, surface_pressure: float, pressure_levels: np.ndarray) -> float:
    ...
    #ED edit: dealing with the clipped arrays that will have nans
    #print('pressure levels', pressure_levels)
    #print('surface pressure', surface_pressure)
    if np.isnan(surface_pressure)==True:
        result=np.nan
    else:
        mask = pressure_levels <= surface_pressure
        y = integrand[mask]
...
def integrate_with_extrapolation(
    ...
    if check_finite:
        if not xr.ufuncs.isfinite(da).all():
            #ED edit: print that there are nan values as a check but continue
            print("Integration includes results which are not finite - check no nans in input except for clipped ds")
